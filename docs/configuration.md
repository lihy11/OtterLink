...
- `FEISHU_WS_IDLE_RESTART_MS`: 长连接在该时间内没有收到任何原始事件时，主动重建 WS client；默认 `0`，表示关闭。空闲本身并不代表连接失效，不建议在生产中默认开启
...
- `RUNTIME_MODE`: `acp | exec_json | codex_app_server | hybrid`
  - `hybrid`：`claude_code` 走 ACP，`codex` 走 app-server
  - `acp_fallback` 兼容值当前等价于 `hybrid`
- `ACP_ADAPTER`: `claude_code | codex`，仅在显式使用 ACP runtime 时生效
...
- `codex` 现行默认运行路径是本地 `codex app-server`
- `CODEX_BIN`: `codex` 可执行文件
- `CODEX_WORKDIR`: runtime 工作目录
- `CODEX_MODEL`: 可选模型名
- `CODEX_SKIP_GIT_REPO_CHECK`: 是否跳过 git 检查
- 当前 `codex app-server` 接入固定使用：
  - `approvalPolicy = never`
  - thread 级 `sandbox = danger-full-access`
  - turn 级 `sandboxPolicy = { type = dangerFullAccess }`
  这样 Codex 在执行过程中不会因为审批或沙箱确认卡住
...
5. `/ot load [workspace]` 优先走 runtime 的会话列举能力；`claude_code` 当前优先使用 ACP `session/list`。
6. `codex` 当前优先通过 app-server `thread/list` 列举同 `cwd` 的 threads；只有运行时不支持时才回退读取 `CODEX_HOME_DIR/state_5.sqlite`。
...
11. `/ot stop` 会对当前活动 turn 发停止请求；`claude_code` ACP 走 `session/cancel`，`codex app-server` 走 `turn/interrupt`，`exec_json` 走本地进程终止。
...
14. 当 `codex` 有活跃 turn 时，新的普通文本消息会被 Rust 转成 app-server `turn/steer`。
