...
最新运行控制补充包括：`/ot stop`、Claude ACP 取消、Codex app-server `turn/steer` / `thread/list` / `thread/read`、会话发现 fallback、代理注入策略、飞书入站 `message_id` 去重、gateway 薄转发边界，以及本地 `otterlink` 控制台工具与一键安装脚本。
